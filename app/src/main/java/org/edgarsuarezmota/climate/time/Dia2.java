...
public class Dia2 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        bar.getMenu().findItem(R.id.page_3).setChecked(true);
    ...
    private void makeApiCall() {
        ...
        if (ip == null) {
            call = weatherApiService.getWeather(API_KEY, lat + "," + lon, deviceLanguage,3);
        } else {
            call = weatherApiService.getWeather(API_KEY, ip, deviceLanguage,3);
        }
            ...
            public void onResponse(Call<WeatherEntity> call, Response<WeatherEntity> response) {
                if (response.isSuccessful()) {
                    ...
                    if (weatherEntity != null && weatherEntity.getForecast() != null && weatherEntity.getForecast().getForecastday() != null && !weatherEntity.getForecast().getForecastday().isEmpty() && weatherEntity.getForecast().getForecastday().get(0).getHour() != null) {
                        ...
                        hours = weatherEntity.getForecast().getForecastday().get(2).getHour();
    ...
    private void getCurrentWeatherInfo(WeatherApiService weatherApiService) {
        ...
        if (ip == null) {
            currentWeatherCall = weatherApiService.getWeather(API_KEY, lat + "," + lon, deviceLanguage,3);
        } else {
            currentWeatherCall = weatherApiService.getWeather(API_KEY, ip, deviceLanguage,3);
        }
